...
{    
    ...
    if (!left_x_at_Y_offset.empty() && left_x_at_Y_offset.size() <4) {
    ...
    if (!right_x_at_Y_offset.empty() && right_x_at_Y_offset.size() <4) {
    ...
    if(rightC > 300 && leftC == 0){
        rpos.x = 640;
        // rpos.x = lpos.x +500;
        prev_rpos = rpos;
    }
    if(leftC >300&& rightC ==0){
        lpos.x = 0;
        // lpos.x = rpos.x -500;
        prev_lpos = lpos;
    }
